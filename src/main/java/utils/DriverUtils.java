...
public class DriverUtils {
    ...
    public static void scrollThePageUntilElementFound(WebElement element){
        ((JavascriptExecutor) Singletone.getDriver()).executeScript("arguments[0].scrollIntoView(true);", element);
    }

    public static WebDriverWait getWebDriverWait(long millis){
        if (webDriverWait == null)
            return webDriverWait = new WebDriverWait(Singletone.getDriver(), Duration.ofMillis(millis));
        else
